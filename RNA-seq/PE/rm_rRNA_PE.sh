...
#SBATCH -n 8
#SBATCH -t 6:00:00
#SBATCH -J 210721_HiSAT2_rRNA_PE
...
#file paths
FQ_PATH=/proj/trim_fastq/PE
OUTPUT_PATH=/proj/rmrRNA/PE
REF_PATH=/proj/reference/rRNA.dir

#loop to run HiSAT2 alignment to rRNA, and keeping unaligned reads.

cd ${FQ_PATH}
for i in $(ls *_tc_R1.fastq.gz | sed 's/_tc_R1.fastq.gz//g'); do	
	hisat2 \
		-p 8 \
		--rna-strandness RF \
		--un-conc-gz ${OUTPUT_PATH}/${i}_tc_rmrRNA.fastq.gz \
