...
for (i in paste(del_patient_studyID, "_rsem", sep="")) {
  temp_count <- sum(names(get(i)) %in% as.vector(del_patient_corID), drop = FALSE) #since del_patient_corID contains "Entrez_Gene_Id", so need to -1
  temp_df <- c(i, temp_count)
  patient_bothdata <- rbind(patient_bothdata, temp_df)
  rm(temp_count, temp_df)
}
patient_bothdata
#gbm_tcga_pan_can_atlas_2018_rsem only have 4 patient have both datasets. the rest looks fine
...
#all the directory below were: /Users/keyigeng/Dropbox/HAP1_Gen_insta/clinical_relevance/cBioPortal/all_datasets/plots/, I changed to /Users/keyigeng/Dropbox/HAP1_Gen_insta/clinical_relevance/cBioPortal/all_datasets/plots/reproduce/
...
topKEGG_HAP1_DE_genelist_topx <- DE_OR_KEGG_DOWN %>%
   dplyr::slice_min(p.adjust, n=5) %>%  #when it's all, I took 17, when I draw top5, like the heatmap below, it is 5.
   dplyr::select(geneID)
...
#since it is not that significant, I am thinking to change another way to compare. instead of compare to everything that is not -2. can compare to patient that is 0. and in this case, it would be more interesting to look at the radiactive therapy.
```{r}
